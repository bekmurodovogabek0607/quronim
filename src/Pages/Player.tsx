...
const Player = () => {
  ...
  function RepeatAll() {
    let num: number = props?.Audio as number
    if (CurrentTime == props?.audioRef.current?.duration) {
      props.setAudio(num + 1)
    }
    ...
    <div style={props?.PlayerShow ? { display: "block" } : { display: "none" }} className='Audioplayer'>
      ...
      <div>
        <ul>
          ...
          <li style={Repeat ? { fontSize: '15px', color: 'yellow' } : { fontSize: '15px' }} onClick={() => { Repeat ? setRepeat(false) : setRepeat(true) }}><i className="fa-solid fa-repeat"></i></li>
